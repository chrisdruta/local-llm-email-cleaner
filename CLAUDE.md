...
- **One action vocabulary** (keep / archive / trash) across rules, LLM, and runner; per-stage verdict columns (`rule_*`, `llm_*`) with a single final `staged_action` + `decision_source` (rule / llm / rule+llm / human), NULL together or set together. `Action.REVIEW` exists only as the LLM's own "unsure" verdict in `llm_action`. One writer per column: engine → `rule_*`, classifier → `llm_*`, the UI's decide buttons → human decisions, policy/UI/runner → `review_status`. Rule-vs-LLM disagreement leaves the row undecided (`models.finalize` returns NULL/NULL, strict equality); the derived states are `models.AWAITING_LLM_WHERE` / `NEEDS_DECISION_WHERE`. Schema bumps mean a fresh DB (the sole exception: `scripts/migrate_v5_to_v6.py`).
- **Auto-trash** only when ALL hold: staged by a trash rule + LLM-confirmed at high confidence (≥0.90), or no-rule-matched + LLM alone above the higher `auto_llm_only_min_confidence` bar (default 0.95); plus no attachments, not a known contact, no keep-voting rule hit, past the age floor (ephemeral digests flagged by both rule AND LLM only need days, not months) — everything else goes to review. Policy gates are re-runnable; preview and execution share the same SQL.
